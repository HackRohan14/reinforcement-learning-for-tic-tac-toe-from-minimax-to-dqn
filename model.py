...
def check_row_win(board, player):
    ...
    for i in range(0,len(board)):
        count_min1,count_1=0,0
        for j in range(0,len(board)):
            if(board[i][j]==1):
                count_1=count_1+1
            elif(board[i][j]==-1):
                count_min1=count_min1+1
        if player==1 and count_1==3:
            return True
        elif player==-1 and count_min1==3:
            return True
    return False
...
def check_main_diagonal_win(board, player):
    ...
    for i in range(0,len(board)):
        for j in range(0,len(board)):
            if(i==j and board[i][j]==1):
                count_x=count_x+1
            elif(i==j and board[i][j]==-1):
                count_o=count_o+1
    if(player==1 and count_x==3):
        return True
    elif(player==-1 and count_o==3):
        return True
    return False
...
def play_hardcoded_game(moves):
    ...
    for i in moves:
        ...
        col=i[1]
        if(is_cell_empty(board,row,col)):
            board=place_move(board,row,col,curr_player)
            if(get_game_status(board) in ['X_win','O_win','draw']):
...
def play_interactive_game():
    ...
    while(get_game_status(board)=='ongoing'):
        ...
        if(not is_cell_empty(board,row,col)):
            while(not is_cell_empty(board,row,col)):
                print_board(board)
                row_col=input().split()
                row=int(row_col[0])
                col=int(row_col[1])
...
class TicTacToeGame:
    ...
    def legal_moves(self):
        """List all currently legal moves."""
    ...
    def step(self, row, col):
        """
        Play the current player's move and update game state.
        ...
        # If game ends, current_player remains the player
        # who just moved (required by the specification).
...
def random_move_agent(board, player, rng):
    ...
    Args:
        ...
        player: Current player (unused, but kept for API consistency).
        rng: numpy.random.Generator instance.

    Returns:
        (row, col) tuple of Python ints.
    """
...
def play_random_vs_random_matches(n_games, rng):
    ...
    ans_list=[]
    while(n_games>0):
        ans_list.append(play_random_vs_random_game(rng))
        n_games=n_games-1
    return ans_list
...
def compute_outcome_rates(outcomes):
    ...
    x_win = outcomes.count("X_win")
    o_win = outcomes.count("O_win")
    draw = outcomes.count("draw")

    return {
        "x_win_rate": x_win / total,
        "o_win_rate": o_win / total,
...
def minimax_recursive(board, player):
    ...
    if status != "ongoing":
        memo[key] = minimax_terminal_score(status)
        return memo[key]

    if player == 1:
        best = -1
        for row, col in get_legal_moves(board):
            value = minimax_recursive(
                place_move(board, row, col, player),
                switch_player(player)
            )
            best = max(best, value)
...
def minimax_max_min_step(board, player):
    ...
    if player == 1:
        ...
        for row, col in moves:
            ...
            value = minimax_recursive(child, switch_player(player))
...
def minimax_alpha_beta(board, player, alpha, beta):
    ...
    if player == 1:  # Maximizing X
        ...
        for move in get_legal_moves(board):
            ...
            score, _ = minimax_alpha_beta(board, -player, alpha, beta)
...
def epsilon_decay_schedule(initial_epsilon, episode_index, min_epsilon, decay_rate):
    ...
    return max(min_epsilon,initial_epsilon* (np.exp(-1*decay_rate*episode_index)))
...
def epsilon_greedy_select_action(q_table, state_key, legal_actions, epsilon, rng):
    ...
    return max(
        legal_actions,
        key=lambda action: q_table.get((state_key, action), 0.0)
    )
...
def greedy_argmax_over_legal_actions(q_table, state_key, legal_actions, rng):
    ...
    best_actions = [
        action for action, q in zip(legal_actions, q_values)
        if q == max_q
    ]
